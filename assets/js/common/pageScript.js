...
import { TextScript } from '../common/textScript.js';
...
export class PageScript extends TextScript {  constructor(trinkaspaceEngine) {
  ...
  /**
   * Load the advanced script configuration from script_omega.json (preferred) or alphas/script_alpha.json (fallback)
   * @param {string} pageId - The ID of the current page
   */
  async loadPageScriptConfig(pageId) {
    try {
      // Try omega first (CUC-scaled, runtime data)
      let response = await fetch(`/pages/chapter_001/${pageId}/script_omega.json`);
      if (!response.ok) {
        // Fallback to canonical alpha (CUC=1)
        response = await fetch(`/pages/chapter_001/${pageId}/alphas/script_alpha.json`);
        if (!response.ok) {
          console.warn(`[PageScript] No script_omega.json or script_alpha.json found for page ${pageId}`);
          return false;
        }
        this.pageScriptConfig = await response.json();
        this.pageScriptConfig._source = 'alpha';
        console.log('[PageScript] Loaded canonical script_alpha.json:', this.pageScriptConfig);
      } else {
        this.pageScriptConfig = await response.json();
        this.pageScriptConfig._source = 'omega';
        console.log('[PageScript] Loaded script_omega.json:', this.pageScriptConfig);
      }
      ...
      console.error('[PageScript] Error loading script config:', error);
      return false;
    }
  }
    /**
...
/**
 * Simple scroll indicator setup for gallery and info pages
 * @param {string} textContentSelector - Selector for the scrollable text content
 * @param {string} indicatorSelector - Selector for the scroll indicator element
 */
export function setupScrollIndicator(textContentSelector, indicatorSelector) {
  document.addEventListener('DOMContentLoaded', () => {
    const textContent = document.querySelector(textContentSelector);
    const scrollIndicator = document.querySelector(indicatorSelector);
    if (!textContent || !scrollIndicator) return;
    ...
    textContent.addEventListener('scroll', function() {
      if (textContent.scrollHeight - textContent.scrollTop <= textContent.clientHeight + 10) {
        scrollIndicator.style.display = 'none';
      } else {
        scrollIndicator.style.display = 'block';
      }
