...
export class RoyalAlphaDebug {
  ...
  init() {
    ...
    this.overlay.innerHTML = `
      ...
      <table>
        ...
        <tr><td>Origo:</td><td>${this.engine.pageOrigoX.toFixed(0)}×${this.engine.pageOrigoY.toFixed(0)}</td></tr>
      </table>
    ...
    // Create page origo marker
    this.origoMarker = document.createElement('div');
    this.origoMarker.className = 'page-origo-marker';
    this.origoMarker.style.left = `${this.engine.pageOrigoX}px`;
    this.origoMarker.style.top = `${this.engine.pageOrigoY}px`;
    document.body.appendChild(this.origoMarker);
    
    // Setup event listeners for debug controls
    this.setupEventListeners();
    
    // Track mouse position relative to page origo
  ...
  update() {
    ...
    if (this.grid) {
      this.grid.style.backgroundSize = `${this.engine.a1Width / 10}px ${this.engine.a1Width / 10}px`;
      this.grid.style.height = `${this.engine.a1Width * 10}px`;
    }
  }
  ...
  trackMousePosition() {
    ...
    infoRow.innerHTML = '<td>Mouse:</td><td>0×0 (0×0)</td>';
    this.overlay.querySelector('table').appendChild(infoRow);
    
    const infoCell = infoRow.querySelector('td:last-child');
    
    document.addEventListener('mousemove', (e) => {
      if (!this.isActive) return;
      
      const offsetX = e.pageX - this.engine.pageOrigoX;
      const offsetY = e.pageY - this.engine.pageOrigoY;
      
      // Calculate alpha coordinates (reverse the CUC)
      const alphaX = Math.round(offsetX / this.engine.cuc);
      const alphaY = Math.round(offsetY / this.engine.cuc);
      
      infoCell.textContent = `${offsetX.toFixed(0)}×${offsetY.toFixed(0)} (α: ${alphaX}×${alphaY})`;
    });
  }
}
